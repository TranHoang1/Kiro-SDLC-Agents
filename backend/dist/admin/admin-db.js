...
async function seedDefaults(pool) {
    ...
    await pool.query(`INSERT INTO access_groups (access_group_id, access_group_name, is_system_group, created_at, updated_at)
     VALUES ('grp-admin', 'Administrators', 1, $1, $1) ON CONFLICT DO NOTHING`, [now]);
    const allPerms = [
    ...
    for (const perm of allPerms) {
        await pool.query('INSERT INTO group_permissions (access_group_id, permission_id, role_data) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING', ['grp-admin', perm, '{}']);
    }
    const hash = hashPassword('admin');
    await pool.query(`INSERT INTO users (user_id, username, email, password_hash, status, access_group_id, force_password_change, created_at)
     VALUES ('user-admin-001', 'admin', 'admin@localhost', $1, 'ACTIVE', 'grp-admin', 0, $2) ON CONFLICT DO NOTHING`, [hash, now]);
}
// --- Password Hashing ---
...
export async function createSession(userId, device, ip) {
    ...
    await pool.query(`INSERT INTO sessions (session_id, user_id, token, device, ip_address, login_at, expires_at, is_active) VALUES ($1, $2, $3, $4, $5, $6, $7, 1)`, [sessionId, userId, token, device || '', ip || '', now.toISOString(), expires.toISOString()]);
...
export async function validateSession(token) {
    const pool = getPool();
    const row = (await pool.query(`SELECT s.user_id, s.expires_at, s.is_active, u.username, u.access_group_id, u.status
     FROM sessions s JOIN users u ON s.user_id = u.user_id WHERE s.token = $1`, [token])).rows[0];
    if (!row || !row.is_active || row.status !== 'ACTIVE')
        return null;
...
export async function invalidateUserSessions(userId) {
    const result = await getPool().query('UPDATE sessions SET is_active = 0 WHERE user_id = $1 AND is_active = 1', [userId]);
    return result.rowCount ?? 0;
}
export async function refreshSession(token) {
    const pool = getPool();
    const row = (await pool.query(`SELECT s.expires_at, s.is_active, u.status FROM sessions s JOIN users u ON s.user_id = u.user_id WHERE s.token = $1`, [token])).rows[0];
    if (!row || !row.is_active || row.status !== 'ACTIVE')
        return null;
    if (new Date(row.expires_at) < new Date()) {
        await pool.query('UPDATE sessions SET is_active = 0 WHERE token = $1', [token]);
...
// --- User Operations ---
export async function getUsers(filters, page = 1, pageSize = 50) {
    const pool = getPool();
    let pIdx = 1;
    let where = 'WHERE 1=1';
    const params = [];
    if (filters?.status) {
        where += ` AND u.status = $${pIdx++}`;
        params.push(filters.status);
    }
    if (filters?.accessGroupId) {
        where += ` AND u.access_group_id = $${pIdx++}`;
    ...
    const rows = (await pool.query(`SELECT u.*, g.access_group_name FROM users u LEFT JOIN access_groups g ON u.access_group_id = g.access_group_id ${where} ORDER BY u.created_at DESC LIMIT $${pIdx} OFFSET $${pIdx + 1}`, [...params, pageSize, (page - 1) * pageSize])).rows;
    return {
        total,
        items: rows.map(r => ({
...
export async function createUser(username, email, password, accessGroupId) {
    const pool = getPool();
    const userId = 'user-' + crypto.randomUUID().slice(0, 8);
    const now = new Date().toISOString();
    const hash = hashPassword(password);
    await pool.query(`INSERT INTO users (user_id, username, email, password_hash, status, access_group_id, force_password_change, created_at) VALUES ($1, $2, $3, $4, 'ACTIVE', $5, 1, $6)`, [userId, username, email, hash, accessGroupId, now]);
...
export async function updateGroup(groupId, name, permissions) {
    const pool = getPool();
...
// --- Audit Operations ---
export async function recordAudit(userId, username, action, resource, resourceId, changes, ip) {
    const auditId = 'aud-' + crypto.randomUUID().slice(0, 8);
    await getPool().query(`INSERT INTO audit_log (audit_id, user_id, username, action, resource, resource_id, changes, timestamp, ip_address) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [auditId, userId, username, action, resource, resourceId || '', changes || '', new Date().toISOString(), ip || '']);
}
export async function getAuditLogs(filters, page = 1, pageSize = 50) {
    ...
    const rows = (await pool.query(`SELECT * FROM audit_log ${where} ORDER BY timestamp DESC LIMIT $${pIdx} OFFSET $${pIdx + 1}`, [...params, pageSize, (page - 1) * pageSize])).rows;
    return {
        total,
...
export async function recordConfigChange(section, key, oldValue, newValue, changedBy, requiresRestart) {
    const pool = getPool();
    await pool.query(`INSERT INTO config_changes (section, key, old_value, new_value, changed_by, changed_at, requires_restart) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [section, key, oldValue, newValue, changedBy, new Date().toISOString(), requiresRestart ? 1 : 0]);
...
export async function getConfigChanges(limit = 10) {
...
// --- Query Logs ---
export async function recordQueryLog(query, responseTimeMs, resultCount, userId = '') {
    await getPool().query('INSERT INTO query_logs (query, timestamp, response_time_ms, result_count, user_id) VALUES ($1, $2, $3, $4, $5)', [query, new Date().toISOString(), responseTimeMs, resultCount, userId]);
}
export async function getQueryLogs(days = 14, userId) {
    const pool = getPool();
...
export async function getQueryLogStats(userId) {
    const pool = getPool();
    let pIdx = 1;
    const totalParams = [];
    let userFilter = '';
    if (userId) {
        userFilter = ` WHERE user_id = $${pIdx++}`;
    ...
    const last24hParams = [since24h];
    let userFilterAnd = '';
    if (userId) {
        userFilterAnd = ` AND user_id = $2`;
        last24hParams.push(userId);
    }
...
// --- Promotion Cooldown ---
export async function setPromotionCooldown(entryId, rejectedBy) {
    const cooldownUntil = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
    await getPool().query('INSERT INTO promotion_cooldowns (entry_id, cooldown_until, rejected_at, rejected_by) VALUES ($1, $2, $3, $4)', [entryId, cooldownUntil, new Date().toISOString(), rejectedBy]);
}
export async function checkPromotionCooldown(entryId) {
    const now = new Date().toISOString();
    const row = (await getPool().query('SELECT cooldown_until FROM promotion_cooldowns WHERE entry_id = $1 AND cooldown_until > $2 ORDER BY cooldown_until DESC LIMIT 1', [entryId, now])).rows[0];
    if (row)
...
// --- KB Embedding space ---
export async function getKbEmbeddings(limit = 100) {
    try {
        const pool = getPool();
    ...
    `, [limit])).rows;
        ...
        const items = fallbackRows.map((row, i) => {
...
// --- KB single entry ---
export async function getKbEntryById(entryId) {
    try {
...
export async function getKbEntries(page = 1, pageSize = 20, sortBy = 'created_at', sortDir = 'desc') {
    try {
        const pool = getPool();
        const colResult = await pool.query("SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'knowledge_entries'");
        ...
        const rows = (await pool.query(`SELECT * FROM knowledge_entries ORDER BY ${safeSort} ${safeDir} LIMIT $1 OFFSET $2`, [pageSize, (page - 1) * pageSize])).rows;
...
export async function updateKbEntryTags(entryId, tags) {
    try {
...
export async function renameKbTag(oldName, newName) {
    let renamed = 0;
    try {
        const pool = getPool();
...
export async function deleteKbTag(tagName) {
    let removed = 0;
    try {
        const pool = getPool();
...
export async function mergeKbTags(sourceTag, targetTag) {
    let merged = 0;
    try {
        const pool = getPool();
...
export async function getKbEntriesByTag(tagName) {
    const entries = [];
    try {
        ...
        for (const row of rows) {
            if (!row.tags)
                continue;
            if (row.tags.split(',').map((t) => t.trim()).includes(tagName))
                entries.push(row);
        }
    }
    catch (e) {
...
// --- MCP Credentials ---
export async function getMcpCredentials(userId, serverName) {
    try {
        const row = (await getPool().query('SELECT credentials FROM mcp_credentials WHERE user_id = $1 AND server_name = $2', [userId, serverName])).rows[0];
...
export async function setMcpCredentials(userId, serverName, credentials) {
    const now = new Date().toISOString();
    await getPool().query(`INSERT INTO mcp_credentials (user_id, server_name, credentials, updated_at)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (user_id, server_name) DO UPDATE SET credentials = EXCLUDED.credentials, updated_at = EXCLUDED.updated_at`, [userId, serverName, JSON.stringify(credentials), now]);
}
export async function getAllMcpCredentials(userId) {
    try {
        const rows = (await getPool().query('SELECT server_name, credentials FROM mcp_credentials WHERE user_id = $1', [userId])).rows;
