...
`;
export async function runGraphMigrations(pool) {
    ...
    await pool.query("INSERT INTO schema_version (version) VALUES (3) ON CONFLICT (version) DO UPDATE SET version = 3");
    console.error('[graph-migrator] Schema version set to 3');
}
async function addEnhancedSymbolColumns(pool) {
...
export async function isGraphSchemaReady(pool) {
    try {
        const result = await pool.query("SELECT COUNT(*) as cnt FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'relationships'");
