...
export async function runMigrations(pool) {
    ...
    if (current < 1) {
        await pool.query("INSERT INTO schema_version (version) VALUES (1) ON CONFLICT (version) DO NOTHING");
...
async function applyMigrationV4(pool) {
    try {
        for (const table of [...MEMORY_TABLES].reverse()) {
            await pool.query(`DROP TABLE IF EXISTS ${table} CASCADE`);
        }
        await pool.query(MEMORY_SCHEMA);
        await pool.query("INSERT INTO schema_version (version) VALUES (4) ON CONFLICT (version) DO UPDATE SET version = 4");
...
async function applyMigrationV2(pool) {
    try {
        for (const col of MIGRATION_V2_COLUMNS) {
            await pool.query(`ALTER TABLE modules ADD COLUMN IF NOT EXISTS ${col} TEXT DEFAULT NULL`);
        }
        await pool.query("INSERT INTO schema_version (version) VALUES (2) ON CONFLICT (version) DO UPDATE SET version = 2");
