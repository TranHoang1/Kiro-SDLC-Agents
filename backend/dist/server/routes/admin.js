...
export function createAdminRoute(logger) {
     ...
     */
    const requirePermission = async (c, userId, requiredPermission) => {
    ...
    app.get('/api/admin/rbac/groups', async (c) => {
        ...
        const userCounts = await Promise.all(groups.map(g => getPool().query('SELECT COUNT(*) as cnt FROM users WHERE access_group_id = $1', [g.accessGroupId])
            .then(r => [g.accessGroupId, parseInt(r.rows[0].cnt)])));
